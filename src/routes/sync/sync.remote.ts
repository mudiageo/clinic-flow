...
export const pushOperations = command(
  ...
  async (operations) => {
    ...
    for (const op of operations) {
      ...
      const existing = q ? await q.findFirst({
      ...
      for (const [k, v] of Object.entries(cleanPayload)) {
         if (['dob', 'recordedAt', 'calledAt', 'completedAt', 'visitDate', 'dueDate', 'sentAt', 'dispensedAt'].includes(k) && typeof v === 'number') {
             cleanPayload[k] = new Date(v);
         }
      }

      if (!existing || op.timestamp >= (existing.updatedAt?.getTime() || 0)) {
         if (op.entityType === 'queueTickets' && existing && existing.triageLevel === 'red' && payload.triageLevel && payload.triageLevel !== 'red') {
             conflicts.push({ localId: op.localId, reason: 'urgency-override-blocked', serverVersion: existing });
             continue;
         }
         ...
         conflicts.push({ localId: op.localId, reason: 'timestamp-conflict', serverVersion: existing });
      }
    }
...
export const pullChanges = query(
  ...
  async ({ phcId, deviceId, since }) => {
    ...
    const changes = await serverDb.query.syncOperations.findMany({
      where: (t, { and, eq, gt, ne }) => and(
        eq(t.phcId, phcId),
        ne(t.deviceId, deviceId), 
        gt(t.serverReceivedAt, new Date(since))
      ),
