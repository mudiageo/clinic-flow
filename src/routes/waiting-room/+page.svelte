<script lang="ts">
	...
	const calledTickets = $derived(queueStore.sortedQueue.filter((t: any) => t.status === 'called'));
	...
	$effect(() => {
		...
		if (calledTickets.length > 0) {
			const newlyCalled = [...calledTickets].sort(
...
<!-- Fullscreen TV UI -->
<div
	class="min-h-screen w-full flex flex-col bg-background p-4 md:p-8 overflow-hidden animate-fade-in"
>
	<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
		<div class="flex items-center gap-3">
			<div
				class="size-12 rounded-2xl bg-primary text-primary-foreground flex items-center justify-center shadow-lg shadow-primary/30"
			>
				<HeartPulse class="size-6" />
			</div>
			<div>
				<h1 class="text-2xl md:text-4xl font-extrabold text-foreground tracking-tight">
					ClinicFlow Waiting Room
				</h1>
				<p class="text-sm md:text-lg text-muted-foreground mt-0.5 font-medium">
					Please watch this screen and wait for your name to be announced
				</p>
			</div>
		</div>
		...
		<div class="flex flex-wrap items-center gap-3 bg-muted/40 p-3 rounded-xl border border-border">
			<div class="space-y-1">
				<span
					class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1"
				>
				...
				<Select type="single" bind:value={selectedVoiceURI}>
					<SelectTrigger
						class="bg-background border-border text-foreground w-48 md:w-60 h-9 text-xs"
					...
					<SelectContent class="bg-card border-border max-h-48 overflow-y-auto text-xs">
						{#each voices as voice}
							<SelectItem value={voice.voiceURI} class="hover:bg-muted"
			...
			<Button
				variant="outline"
				class="h-9 text-xs border-border text-foreground btn-press self-end"
	...
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0">
		...
		<Card
			class="bg-primary/5 border-primary/20 flex flex-col h-[400px] lg:h-full overflow-hidden card-hover"
		>
			<CardHeader class="bg-primary/10 border-b border-primary/20 py-4 md:py-6">
				<CardTitle
					class="text-primary text-xl md:text-3xl font-extrabold text-center flex items-center justify-center gap-2"
				>
			...
			<CardContent class="p-4 md:p-6 overflow-y-auto flex-1 space-y-4 no-scrollbar">
				{#if calledTickets.length === 0}
					<div
						class="h-full flex flex-col items-center justify-center py-12 text-center text-muted-foreground/40"
					>
				...
				{:else}
					{#each calledTickets as ticket}
						...
						<div
							class="bg-primary/10 border border-primary/20 p-5 md:p-6 rounded-2xl flex items-center justify-between animate-fade-in shadow-md shadow-primary/5"
						>
							<div>
								<div class="text-2xl md:text-4xl font-extrabold text-foreground tracking-tight">
									{p?.name || 'Unknown Patient'}
								</div>
								<div class="text-sm md:text-lg text-primary mt-1 font-mono font-bold">
									{p?.clinicId}
								</div>
							</div>
							<div class="text-right">
								<div class="text-xs md:text-sm font-bold text-primary uppercase tracking-wider">
									Proceed To
								</div>
								<div class="text-xl md:text-3xl font-black text-foreground mt-0.5 tracking-tight">
									DOCTOR DESK
								</div>
							</div>
						</div>
		...
		<Card class="overflow-hidden bg-card/60 flex flex-col h-[400px] lg:h-full card-hover">
			<CardHeader class="bg-muted/20 border-b border-border/60 py-4 md:py-6">
				<CardTitle
					class="text-foreground text-lg md:text-2xl font-extrabold text-center flex items-center justify-center gap-2"
				>
			...
			<CardContent class="p-4 md:p-6 overflow-hidden flex-1 flex flex-col">
				<ScrollArea class="h-full w-full pr-1">
					<div class="grid grid-cols-1 gap-3">
						{#if waitingTickets.length === 0}
							<div
								class="py-16 text-center text-muted-foreground/60 flex flex-col items-center justify-center"
							>
						...
						{:else}
							{#each waitingTickets.slice(0, 10) as ticket, i}
								{@const p = patientStore.get(ticket.patientId)}
								<div
									class="flex items-center justify-between p-4 bg-background/50 rounded-xl border border-border/60 hover:bg-muted/40 transition-colors"
								>
									<div class="flex items-center gap-3">
										<div
											class="w-8 h-8 rounded-full bg-muted text-muted-foreground flex items-center justify-center font-bold text-sm"
										>
											{i + 1}
										</div>
										<div>
											<div class="text-base md:text-lg font-bold text-foreground tracking-tight">
												{p?.name || 'Unknown Patient'}
											</div>
									...
									<div>
										{#if ticket.triageLevel === 'red'}
											<Badge
												class="bg-triage-red/15 text-triage-red border-triage-red/25 hover:bg-triage-red/15 px-2 py-0.5 text-[10px] font-bold tracking-wider uppercase animate-pulse"
												>Priority</Badge
											>
										{/if}
									</div>
							...
							{#if waitingTickets.length > 10}
								<div
									class="text-center pt-4 pb-2 text-xs font-bold text-muted-foreground tracking-wider uppercase"
								>
