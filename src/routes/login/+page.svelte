...
<div class="min-h-screen w-full bg-background flex flex-col items-center justify-center p-4 relative overflow-hidden animate-fade-in">
  ...
  <div class="w-full max-w-md relative z-10 space-y-6">
    <div class="text-center">
      <div class="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-primary text-primary-foreground shadow-lg shadow-primary/30 mb-4 animate-bounce">
        <HeartPulse class="w-7 h-7" />
      </div>
      <h1 class="text-3xl font-extrabold text-foreground tracking-tight">ClinicFlow</h1>
      <p class="text-muted-foreground mt-1.5 text-sm font-medium">Oredo Primary Health Centre · Benin City, Nigeria</p>
    </div>
    ...
    <Card class="bg-card/60 border-border/80 backdrop-blur-xl shadow-2xl">
      <CardHeader class="pb-4">
        ...
        <CardDescription class="text-muted-foreground">Access the clinic management platform</CardDescription>
      </CardHeader>
      <CardContent>
        <form {...signInAction} class="space-y-4">
          <!-- Form-level validation errors using shadcn Alert -->
          {#if allIssues.length > 0}
            <Alert variant="destructive" class="py-3 bg-destructive/10 border-destructive/20 text-destructive rounded-xl">
              <AlertCircle class="size-4" />
              <AlertTitle class="text-xs font-bold uppercase tracking-wider">Validation Error</AlertTitle>
          ...
          <Button type="submit" class="w-full h-11 bg-primary text-primary-foreground hover:bg-primary/95 font-semibold rounded-xl shadow-lg shadow-primary/15 transition-all duration-200 mt-2 btn-press">
            Sign In
          </Button>
        </form>
      </CardContent>
