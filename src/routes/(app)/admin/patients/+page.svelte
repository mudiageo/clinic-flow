					...
					{:else}
						{#each results as patient (patient.id)}
							<TableRow class="hover:bg-muted/40 border-b border-border transition-colors group">
								...
								>
								<TableCell class="text-muted-foreground px-6 py-4 text-sm"
									>{patient.dob ?? '—'}</TableCell
								>
								<TableCell class="text-muted-foreground px-6 py-4 text-sm"
									>{patient.community ?? '—'}</TableCell
								>
								<TableCell class="px-6 py-4 text-right">
									<Dialog>
										<DialogTrigger
											class="inline-flex items-center justify-center rounded-xl font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 border border-border bg-background text-muted-foreground hover:text-foreground hover:bg-muted btn-press h-8 px-3 text-xs"
										>
