<script lang="ts">
  ...
  import {
    ClipboardList, UserPlus, Thermometer, Bell,
    Search, Settings, Sun, Moon, Monitor
  } from '@lucide/svelte';
  ...
  const navGroups: NavGroup[] = $derived([
    ...
    },
    {
      label: 'Reminders',
      items: [
        { href: '/nurse/reminders', label: 'SMS Reminders', icon: Bell },
      ],
    },
    {
      label: 'Account',
  ...
  const bottomNavItems: BottomNavItem[] = $derived([
    { href: '/nurse', label: 'Queue', icon: ClipboardList, badge: waitingCount, badgeVariant: waitingCount > 0 ? 'default' : undefined },
