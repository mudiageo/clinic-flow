<script lang="ts">
  ...
  import { vitalsStore, type LocalTriageRule } from '$lib/state/vitals.svelte';
  import { queueStore } from '$lib/state/queue.svelte';
  import { getTriageRules } from '$lib/remote/encounters.remote';
  ...
  // Active triage rules loaded from server
  let rules = $state<LocalTriageRule[]>([]);

  onMount(async () => {
    try {
      // Load active rules for this PHC (using stub UUID)
      const res = await getTriageRules('00000000-0000-0000-0000-000000000000');
      rules = res as any[];
    } catch (err) {
      console.error('Failed to load triage rules from server:', err);
    }
  });
  ...
  const triageResult = $derived(
    vitalsStore.evaluateTriage(
      currentVitals,
      rules,
      selectedPatient?.isPregnant ?? false
    )
  );
    ...
    try {
      ...
      if (activeTickets.length > 0) {
        for (const ticket of activeTickets) {
          await queueStore.applyTriageFlag(
            ticket.id,
            triageResult.level,
            triageResult.reason
          );
...
<div class="max-w-4xl mx-auto space-y-8">
  <div>
    ...
    <p class="text-slate-400 mt-1">Record vitals and automatically determine patient urgency level</p>
  </div>
        ...
        <CardContent class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="space-y-2">
              ...
              <Input id="temp" type="number" step="0.1" bind:value={temperature} placeholder="36.5" class="h-12 bg-slate-950/50 border-slate-800 text-white" />
            </div>

            <div class="space-y-2">
              <Label for="pulse" class="text-slate-300">Heart Rate / Pulse (BPM)</Label>
              <Input id="pulse" type="number" bind:value={pulse} placeholder="72" class="h-12 bg-slate-950/50 border-slate-800 text-white" />
            </div>

            <div class="space-y-2">
              <Label for="systolic" class="text-slate-300">Systolic BP (mmHg)</Label>
              <Input id="systolic" type="number" bind:value={systolicBp} placeholder="120" class="h-12 bg-slate-950/50 border-slate-800 text-white" />
            </div>

            <div class="space-y-2">
              <Label for="diastolic" class="text-slate-300">Diastolic BP (mmHg)</Label>
              <Input id="diastolic" type="number" bind:value={diastolicBp} placeholder="80" class="h-12 bg-slate-950/50 border-slate-800 text-white" />
            </div>

            <div class="space-y-2">
              <Label for="spo2" class="text-slate-300">Oxygen Saturation - SpO2 (%)</Label>
              <Input id="spo2" type="number" bind:value={spo2} placeholder="98" class="h-12 bg-slate-950/50 border-slate-800 text-white" />
            </div>

            <div class="space-y-2">
              <Label for="weight" class="text-slate-300">Weight (kg)</Label>
              <Input id="weight" type="number" step="0.1" bind:value={weight} placeholder="70" class="h-12 bg-slate-950/50 border-slate-800 text-white" />
            </div>
          </div>
          ...
          {#if triageResult}
            ...
            <div class="p-4 rounded-xl border transition-all duration-300 flex flex-col justify-center items-center text-center
