	...
	<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
		<div>
			...
			<p class="text-muted-foreground text-sm">
				Search and view complete patient clinical records.
			</p>
		</div>
	</div>
	...
	<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
		{#each filteredPatients as p (p.id)}
			<a href={`/doctor/patients/${p.id}`} class="block group">
				<Card
					class="h-full bg-card/50 hover:bg-card/80 transition-all border-border/50 hover:border-primary/30 card-hover"
				>
					<CardContent class="p-5 flex items-start gap-4">
						<div
							class="size-10 rounded-full bg-primary/10 flex items-center justify-center text-primary shrink-0 group-hover:scale-110 transition-transform"
						>
							<User class="size-5" />
						</div>
						<div class="flex-1 min-w-0">
							...
							<div class="flex items-center gap-2 mt-1">
								<Badge
									variant="secondary"
									class="font-mono text-[10px] uppercase bg-muted text-muted-foreground"
									>{p.clinicId}</Badge
								>
							</div>
		...
		{#if filteredPatients.length === 0}
			<div
				class="col-span-full py-20 text-center text-muted-foreground border border-dashed rounded-xl bg-card/30"
			>
