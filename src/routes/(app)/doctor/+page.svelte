...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
              ...
              >
                ...
                <div class="flex items-center gap-2">
                  {#if ticket.triageLevel === 'red'}
                    <Badge class="bg-triage-red text-triage-red-foreground font-semibold px-2 py-0.5 hover:bg-triage-red">RED</Badge>
                  {:else}
  ...
  <div class="lg:col-span-2 space-y-6">
    {#if selectedTicket}
      <Card class="bg-card/60 card-hover overflow-hidden">
        <CardHeader class="border-b border-border/60 bg-muted/20 flex flex-row items-center justify-between py-4">
          <div>
            <CardTitle class="text-lg font-bold text-foreground">{patient?.name}</CardTitle>
            <CardDescription class="mt-0.5 font-medium">{patient?.clinicId} • Sex: <span class="capitalize">{patient?.sex}</span></CardDescription>
          </div>
          <div>
            {#if selectedTicket.triageLevel === 'red'}
              <Badge class="bg-triage-red text-triage-red-foreground glow-triage-red font-semibold animate-pulse">RED - IMMEDIATE</Badge>
            {:else if selectedTicket.triageLevel === 'amber'}
              <Badge class="bg-triage-amber text-triage-amber-foreground font-semibold">AMBER - STABLE ALERT</Badge>
            {:else}
              <Badge class="bg-triage-green text-triage-green-foreground font-semibold">GREEN - STABLE</Badge>
            {/if}
          </div>
        </CardHeader>
        ...
        <CardContent class="pt-6 space-y-6">
          <!-- Vitals Summary Card -->
          {#if vitals}
            <div class="p-4 bg-background/50 border border-border/60 rounded-xl grid grid-cols-3 md:grid-cols-6 gap-3 text-center">
              <div class="flex flex-col items-center">
                <p class="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">Temp</p>
                <p class="text-base font-extrabold text-foreground mt-0.5">{vitals.temperatureCelsius ?? '—'}°C</p>
                <Sparkline data={tempTrend} width={60} height={18} color="oklch(from var(--destructive) l c h)" />
              </div>
              <div class="flex flex-col items-center">
                <p class="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">BP</p>
                <p class="text-base font-extrabold text-foreground mt-0.5">{vitals.systolicBp ?? '—'}/{vitals.diastolicBp ?? '—'}</p>
                <Sparkline data={bpSysTrend} width={60} height={18} color="oklch(from var(--primary) l c h)" />
              </div>
              <div class="flex flex-col items-center">
                <p class="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">Pulse</p>
                <p class="text-base font-extrabold text-foreground mt-0.5">{vitals.pulseBpm ?? '—'} BPM</p>
                <Sparkline data={pulseTrend} width={60} height={18} color="oklch(from var(--accent) l c h)" />
              </div>
              <div class="flex flex-col items-center">
                <p class="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">SpO2</p>
                <p class="text-base font-extrabold text-foreground mt-0.5">{vitals.spo2Percent ?? '—'}%</p>
                <Sparkline data={spo2Trend} width={60} height={18} color="oklch(from var(--triage-green) l c h)" />
              </div>
              <div class="flex flex-col items-center">
                <p class="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">Weight</p>
                <p class="text-base font-extrabold text-foreground mt-0.5">{vitals.weightKg ?? '—'} kg</p>
                <Sparkline data={weightTrend} width={60} height={18} color="oklch(from var(--triage-amber) l c h)" />
              </div>
              <div class="flex flex-col items-center justify-center border-l border-border/60 pl-2">
                <p class="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">Triage Reason</p>
                <p class="text-xs font-semibold text-primary mt-1 truncate px-1 max-w-[90px]" title={vitals.triageReason ?? 'Normal'}>
          ...
          <div class="space-y-4">
            <div class="space-y-2">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  {#if isRecording}
                    <AudioWave playing={true} bars={6} class="h-5 w-8 mr-1" barColor="bg-destructive" />
                  {/if}
                  <Button 
              ...
              {#if isRecording || aiProcessing}
                <p class="text-xs text-muted-foreground italic">
                  {isRecording ? 'Listening: ' + (interimTranscript || '...') : 'AI model structuring voice recording into clinical intake format...'}
                </p>
              {/if}
            </div>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              ...
              <div class="space-y-2">
                ...
                <Input type="number" bind:value={quantity} min={1} class="bg-background/50 border-border text-foreground h-10" />
              </div>

              <Button onclick={handleAddMed} class="bg-secondary text-secondary-foreground hover:bg-muted btn-press h-10">
                ...
                <Table>
                  <TableHeader class="bg-muted/30">
                    <TableRow class="hover:bg-transparent">
                      <TableHead class="font-semibold text-muted-foreground px-4 py-2.5 text-xs uppercase tracking-wider">Item Name</TableHead>
                      <TableHead class="font-semibold text-muted-foreground px-4 py-2.5 w-24 text-xs uppercase tracking-wider">Quantity</TableHead>
                      <TableHead class="font-semibold text-muted-foreground px-4 py-2.5 text-right w-24 text-xs uppercase tracking-wider">Action</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {#each prescriptions as item, idx}
                      <TableRow class="border-border hover:bg-muted/40 transition-colors">
                        <TableCell class="text-foreground px-4 py-2.5 font-medium">{item.name}</TableCell>
                        <TableCell class="text-foreground px-4 py-2.5 font-bold tabular-nums">{item.quantity}</TableCell>
                        <TableCell class="text-right px-4 py-2.5">
                          <Button variant="ghost" size="sm" class="text-destructive hover:text-destructive hover:bg-destructive/10 btn-press h-8 px-2" onclick={() => handleRemoveMed(idx)}>
          ...
          <div class="space-y-4">
            ...
            {#if !patient?.phone}
               <div class="p-3 bg-accent/10 border border-accent/20 rounded-xl text-accent-foreground text-xs flex items-center gap-2">
            ...
            {:else}
              <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div class="space-y-2">
                  ...
                  <Select type="single" bind:value={reminderType}>
                    ...
                    <SelectContent class="bg-card border-border">
                      ...
                      <SelectItem value="immunization" class="hover:bg-muted">Immunization</SelectItem>
                ...
                <div class="md:col-span-2 space-y-2">
                  ...
                  <Input bind:value={reminderLabel} placeholder="e.g. Come back for your follow-up check..." class="bg-background/50 border-border text-foreground h-10" />
                </div>
                <div class="space-y-2">
                  <Label class="text-foreground">Days from now</Label>
                  <Input type="number" bind:value={reminderDays} min={1} class="bg-background/50 border-border text-foreground h-10" />
                </div>
              </div>
            {/if}
          </div>
          ...
          <div class="flex justify-end gap-3 pt-4 border-t border-border/60">
            <Button onclick={() => selectedTicket = null} variant="outline" class="border-border text-muted-foreground btn-press h-10 px-5">
              Discard
            </Button>
            <Button onclick={handleCompleteEncounter} class="bg-primary text-primary-foreground hover:bg-primary/95 shadow-md shadow-primary/10 btn-press h-10 px-6">
    ...
    {:else}
      <div class="min-h-[400px] flex flex-col items-center justify-center border border-dashed border-border rounded-xl p-8 text-muted-foreground bg-card/25 animate-pulse text-center">
        <Stethoscope class="size-10 text-muted-foreground/40 mb-3" />
        <span class="text-sm font-medium">Select a patient from the waiting room queue to begin consultation</span>
      </div>
    {/if}
  </div>
</div>
