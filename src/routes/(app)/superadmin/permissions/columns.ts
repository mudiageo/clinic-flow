...
export const columns: ColumnDef<PermissionAuditItem>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const permSnippet = createRawSnippet<[{ permission: string }]>((getPerm) => {
        ...
        return {
          render: () => `<code class="px-1.5 py-0.5 rounded bg-muted text-xs font-mono">${permission}</code>`
        };
      });
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      else if (row.original.expiresAt && new Date(row.original.expiresAt) < new Date()) status = 'Expired';
      ...
      const badgeSnippet = createRawSnippet<[{ status: string }]>((getStatus) => {
        ...
        else if (status === 'Revoked') classes = 'bg-destructive hover:bg-destructive text-destructive-foreground';

        return {
          render: () => `<div class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 ${classes}">${status}</div>`
        };
      });
