<script lang="ts">
	...
	const navGroups: NavGroup[] = [
		{
			label: 'Dispensing',
			items: [{ href: '/pharmacy/dispense', label: 'Dispense Queue', icon: Pill }]
		},
		{
			label: 'Inventory',
			items: [
		...
		{
			label: 'Reports',
			items: [{ href: '/pharmacy/reports', label: 'Usage & Expiry', icon: BarChart3 }]
		},
		{
			label: 'Account',
			items: [{ href: '/pharmacy/settings', label: 'Settings', icon: Settings }]
		}
	];
...
<Sidebar.Provider>
	<AppSidebar {navGroups} {role} userName={user?.name ?? 'Pharmacy'} {userInitials} />
