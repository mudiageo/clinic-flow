...
<div class="space-y-8 animate-fade-in">
  <div class="flex items-start gap-3">
    ...
    <div>
      ...
      <p class="text-muted-foreground text-sm mt-0.5 font-medium">Dispense queue and inventory alerts</p>
    </div>
  </div>
    ...
    <div class="lg:col-span-2 space-y-4">
      ...
      {#if dispenseQueue.length === 0}
        <Card class="bg-card/60 card-hover">
          <CardContent class="py-16 text-center text-muted-foreground flex flex-col items-center justify-center">
          ...
          <Card class="bg-card/60 card-hover mb-4">
            <CardHeader class="pb-3 border-b border-border/60 bg-muted/10">
              <CardTitle class="text-sm font-semibold flex items-center justify-between">
                ...
                <Badge variant="secondary" class="font-semibold text-xs">{queueItem.items.length} items</Badge>
              </CardTitle>
            </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow class="hover:bg-transparent">
                    <TableHead class="font-semibold text-muted-foreground text-xs uppercase tracking-wider">Medication</TableHead>
                    <TableHead class="font-semibold text-muted-foreground text-xs uppercase tracking-wider">Dosage</TableHead>
                    <TableHead class="font-semibold text-muted-foreground text-right text-xs uppercase tracking-wider">Qty</TableHead>
                    <TableHead class="font-semibold text-muted-foreground text-right text-xs uppercase tracking-wider">Action</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {#each queueItem.items as p}
                    <TableRow class="border-border hover:bg-muted/40 transition-colors">
                      ...
                      <TableCell class="text-foreground font-bold text-right tabular-nums">{p.quantity}</TableCell>
      ...
      <Card class="bg-destructive/5 border-destructive/20 card-hover">
        <CardContent class="p-0">
          {#if lowStockItems.length === 0}
            <div class="p-8 text-center text-muted-foreground flex flex-col items-center justify-center">
          ...
          {:else}
            <div class="divide-y divide-destructive/10">
              {#each lowStockItems as item}
                <div class="p-4 flex items-center justify-between hover:bg-destructive/10 transition-colors">
                  <div>
                    <div class="text-foreground font-semibold text-sm flex items-center gap-1.5">
                      {item.itemName}
                      {#if item.isCritical}
                        <Badge variant="destructive" class="px-1.5 py-0 text-[9px] font-bold tracking-wider uppercase">CRITICAL</Badge>
                      {/if}
                    </div>
                    <div class="text-xs text-muted-foreground mt-1">
                      Stock: <span class="font-bold text-destructive">{item.currentStock} {item.unit}</span> (Min: {item.lowStockThreshold})
                    </div>
                  </div>
                  <Button 
                    size="sm" 
