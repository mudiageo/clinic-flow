...
test('seed database', async () => {
	console.log('🌱 Starting database seeding in Benin City context via Vitest...');
	...
	for (const staffMember of staffData) {
		...
		if (!authUser) {
			console.log(`Creating auth user and credentials for ${staffMember.name}...`);
			...
			// Insert into Better Auth's account table
			await db.insert(account).values({
				id: crypto.randomUUID(),
				accountId: staffMember.email,
				providerId: 'email',
				userId: userId,
				password: hashedPassword
			});

			authUser = { id: userId, name: staffMember.name, email: staffMember.email } as any;
		}
	...
	const inventoryItems = [
		{
			itemName: 'Artemether/Lumefantrine 80/480mg (Coartem)',
		...
		{
			itemName: 'Ferrous Sulfate 200mg',
			category: 'supplement',
			unit: 'tablet',
			currentStock: 1000,
			lowStockThreshold: 100,
		...
		{
			itemName: 'Oral Polio Vaccine (OPV)',
			category: 'vaccine',
			unit: 'dose',
			currentStock: 100,
			lowStockThreshold: 20,
	...
	const rules = [
		...
		// Blood Pressure Rules
		{
			...
			threshold: 160,
			resultingLevel: 'red' as const,
			reasonTemplate: 'Severe Hypertension: BP {systolicBp}/{diastolicBp} mmHg',
			requiresPregnant: false
		},
		{
			field: 'diastolicBp',
		...
		},
		{
			...
			threshold: 140,
			resultingLevel: 'amber' as const,
			reasonTemplate: 'Moderate Hypertension: BP {systolicBp}/{diastolicBp} mmHg',
			requiresPregnant: false
		},
		{
			field: 'diastolicBp',
		...
		// Blood Pressure Rules (Pregnancy-specific)
		{
			...
			threshold: 130,
			resultingLevel: 'red' as const,
			reasonTemplate: 'Pregnancy Hypertension Alert: BP {systolicBp}/{diastolicBp} mmHg',
		...
		// SpO2 Rules
		{
			...
			threshold: 90,
			resultingLevel: 'red' as const,
			reasonTemplate: 'Severe Hypoxia: SpO2 {spo2Percent}%',
			requiresPregnant: false
		},
		{
			field: 'spo2Percent',
			...
			threshold: 95,
			resultingLevel: 'amber' as const,
			reasonTemplate: 'Mild Hypoxia: SpO2 {spo2Percent}%',
			requiresPregnant: false
		},
		...
		// Pulse Rules
		{
			...
			threshold: 120,
			resultingLevel: 'red' as const,
			reasonTemplate: 'Severe Tachycardia: Pulse {pulseBpm} bpm',
			requiresPregnant: false
		},
		{
			field: 'pulseBpm',
			...
			threshold: 50,
			resultingLevel: 'red' as const,
			reasonTemplate: 'Severe Bradycardia: Pulse {pulseBpm} bpm',
			requiresPregnant: false
		},
		{
			field: 'pulseBpm',
			...
			threshold: 100,
			resultingLevel: 'amber' as const,
			reasonTemplate: 'Mild Tachycardia: Pulse {pulseBpm} bpm',
			requiresPregnant: false
		},
		{
			field: 'pulseBpm',
			...
			threshold: 60,
			resultingLevel: 'amber' as const,
			reasonTemplate: 'Mild Bradycardia: Pulse {pulseBpm} bpm',
			requiresPregnant: false
		}
	];
	...
	for (const rule of rules) {
		...
		if (!existingRule) {
			...
			console.log(
				`Added triage rule: ${rule.field} ${rule.operator} ${rule.threshold} -> ${rule.resultingLevel}`
			);
		}
	}
	...
	const familyData = [
		{
			...
			members: [
				{
					fullName: 'Osagie Ighodaro',
					phone: '2348011112222',
					dob: new Date('1981-05-14'),
					sex: 'male' as const,
					clinicId: 'OR-IGHO-01'
				},
				{
					fullName: 'Efe Ighodaro',
					phone: '2348011112223',
				...
				{
					fullName: 'Efosa Ighodaro',
					phone: null,
					dob: new Date('2014-11-02'),
					sex: 'female' as const,
					clinicId: 'OR-IGHO-03'
				},
				{
					fullName: 'Osahon Ighodaro',
					phone: null,
		...
		},
		{
			...
			members: [
				...
				{
					fullName: 'Osasere Osagie',
					phone: null,
					dob: new Date('2025-06-05'),
					sex: 'female' as const,
					clinicId: 'OR-OSAG-03'
				}
			]
		...
		},
		{
			...
			members: [
				...
				{
					fullName: 'Itohan Igbinosa',
					phone: null,
					dob: new Date('2020-04-10'),
					sex: 'female' as const,
					clinicId: 'OR-IGBI-02'
				},
				{
		...
		},
		{
			...
			members: [
				{
					fullName: 'Victor Imasuen',
					phone: '2348055556666',
					dob: new Date('1964-01-18'),
					sex: 'male' as const,
					clinicId: 'OR-IMAS-01'
				},
				{
					fullName: 'Omono Imasuen',
					phone: '2348055556667',
					dob: new Date('1971-06-25'),
					sex: 'female' as const,
					clinicId: 'OR-IMAS-02'
				}
			]
