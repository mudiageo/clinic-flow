...
export const permissions = pgTable(
	...
	(table) => ({
		staffPermissionIdx: index('permissions_staff_perm_idx').on(table.staffId, table.permission),
		phcIdx: index('permissions_phc_idx').on(table.phcId)
	})
);
...
export const queueTickets = pgTable(
	...
	(table) => ({
		...
		ticketNumUnique: uniqueIndex('queue_ticket_num_unique').on(
			table.phcId,
			table.ticketNumber,
			table.createdAt
		)
...
export const permissionsRelations = relations(permissions, ({ one }) => ({
	...
	grantedByStaff: one(staff, { fields: [permissions.grantedBy], references: [staff.id] }),
	phc: one(phcs, { fields: [permissions.phcId], references: [phcs.id] })
}));
