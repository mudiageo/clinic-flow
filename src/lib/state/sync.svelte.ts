...
class SyncStore {
  ...
  async flush() {
    ...
    // --- PULL ---
    try {
      const { changes, newCursor } = await pullChanges({ phcId: this.phcId, deviceId: this.deviceId, since: this.cursor });
         ...
         if (table) {
             ...
             if (change.operation === 'create' || change.operation === 'update') {
                 ...
                 for (const [k, v] of Object.entries(payload)) {
                    if (['dob', 'recordedAt', 'calledAt', 'completedAt', 'visitDate', 'dueDate', 'sentAt', 'dispensedAt'].includes(k) && typeof v === 'string') {
