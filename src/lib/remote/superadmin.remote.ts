...
export const getPhcList = query(async () => {
    ...
    for (const phc of allPhcs) {
        const staffCount = await db.select({ value: count() }).from(staff).where(eq(staff.phcId, phc.id));
        const patientCount = await db.select({ value: count() }).from(patients).where(eq(patients.phcId, phc.id));
