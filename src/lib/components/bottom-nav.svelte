...
>
	{#each items.slice(0, 5) as item (item.href)}
		...
		>
			...
			<span
				class={cn('text-[10px] font-medium transition-all', active ? 'opacity-100' : 'opacity-80')}
			>
				{item.label}
			</span>
			...
			{#if active}
				<span
					class="absolute -top-[1px] left-1/2 h-[2px] w-8 -translate-x-1/2 rounded-full bg-primary shadow-[0_0_8px_0_rgba(var(--primary),0.4)]"
				></span>
			{/if}
		</a>
	{/each}
